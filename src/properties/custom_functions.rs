...
use crate::{get_var, set_var, utils, ChainConfig};
...
pub fn execute_custom_function(
    val: &Value,
) -> Result<HashMap<String, Value>, error::PropertyError> {
    ...
    for variable_name in sorted {
        match crate::build_ast_root(properties.get(&variable_name).unwrap().as_str().unwrap()) {
            Ok(root) => match root.evaluate() {
                Ok(r) => {
    ...
    //                 cc =
    //                     serde_json::from_str(&fs::read_to_string(path.as_path()).unwrap()).unwrap();
    ...
    //     let mut function_json: Value =
    //         serde_json::from_str(std::fs::read_to_string(&function_path).unwrap().as_str())
    //             .unwrap();
    ...
    if path.is_empty() {
        ...
        match value {
            Value::Object(map) => {
                ...
                map.get(key)
                    .and_then(|v| find_value_by_path(v, remaining_path))
            }
            Value::Array(arr) => {
...
) -> Result<Vec<String>, error::PropertyError> {
